...
(function () {
    'use strict';
    angular.module('crams').controller('CramsMyAllocationsController', CramsMyAllocationsController);

    CramsMyAllocationsController.$inject = ['FlashService', 'CramsApiService', 'LookupService', '$scope', '$rootScope', '$filter', '$anchorScroll', 'ENV'];

    function CramsMyAllocationsController(FlashService, CramsApiService, LookupService, $scope, $rootScope, $filter, $anchorScroll, ENV) {
        var vm = this;
        vm.loaded = false;
        ...
        CramsApiService.listMyAllocations().then(function (response) {
            if (response.success) {
                vm.projects = response.data;

                //filter the project by system
            ...
            if (vm.selected_status.length !== 0) {
                angular.forEach(vm.projects, function (project) {
                    ...
                    if (requests !== undefined && requests.length > 0) {
                        var req = requests[0];
                        if (_.contains(vm.selected_status, req.request_status.code)) {
